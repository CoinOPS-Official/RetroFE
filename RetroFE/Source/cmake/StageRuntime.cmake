function(retrofe_stage_runtime target)
    ...
    if(RETROFE_ENABLE_FFMPEG)
        list(PREPEND runtime_dirs "${FFMPEG_ROOT}/bin")
        file(GLOB ffmpeg_dlls "${FFMPEG_ROOT}/bin/avcodec-*.dll" "${FFMPEG_ROOT}/bin/avformat-*.dll"
            "${FFMPEG_ROOT}/bin/avutil-*.dll" "${FFMPEG_ROOT}/bin/swresample-*.dll" "${FFMPEG_ROOT}/bin/swscale-*.dll")
        list(APPEND runtime_dlls ${ffmpeg_dlls})
        foreach(notice LICENSE.txt LICENSE)
            if(EXISTS "${FFMPEG_ROOT}/${notice}")
                list(APPEND runtime_licenses "${FFMPEG_ROOT}/${notice}|licenses/FFmpeg/${notice}")
            endif()
        endforeach()
    endif()
