...
void GStreamerVideo::processNewBuffer(GstElement const* /* fakesink */, GstBuffer* buf, GstPad* new_pad, gpointer userdata) {
	...
	if (video && video->isPlaying_) {
		...
		// If buffer layout is unknown, determine it.
		if (video->bufferLayout_ == UNKNOWN) {
			video->videoMeta_ = gst_buffer_get_video_meta(buf);
			if (!video->videoMeta_ || video->useD3dHardware_ || video->useVaHardware_) {
				video->bufferLayout_ = CONTIGUOUS;
				if (video->useD3dHardware_ || video->useVaHardware_) {
					video->expectedBufSize_ = video->width_ * video->height_ + video->width_ * (video->height_ / 2); // NV12 format
				}
				else {
					video->expectedBufSize_ = video->width_ * video->height_ * 3 / 2; // I420 format
				}
			}
			else {
				video->bufferLayout_ = NON_CONTIGUOUS;
				video->expectedBufSize_ = video->width_ * video->height_ * 3 / 2; // I420 format
			}
		}
...
gint64 GStreamerVideo::getCurrent()
{
	gint64 ret = 0;
