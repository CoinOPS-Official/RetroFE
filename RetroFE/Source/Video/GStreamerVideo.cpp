...
void GStreamerVideo::processNewBuffer(GstElement const * /* fakesink */,
                                      GstBuffer *buf, GstPad *new_pad,
                                      gpointer userdata) {
    ...
    if (!video->width_ || !video->height_) {
      ...
      if (caps) {
        ...
        if (g_strcmp0(format, "NV12") == 0) {
          ...
          gst_video_info_set_format(&video->videoInfo_, GST_VIDEO_FORMAT_NV12,
                                    video->width_, video->height_);
          LOG_INFO("Video", "Pixel Format : NV12");
        } else if (g_strcmp0(format, "I420") == 0) {
          video->sdlFormat_ = SDL_PIXELFORMAT_IYUV;
          gst_video_info_set_format(&video->videoInfo_, GST_VIDEO_FORMAT_I420,
                                    video->width_, video->height_);
        ...
        if (video->sdlFormat_ != SDL_PIXELFORMAT_UNKNOWN && video->height_ &&
            video->width_) {
          video->texture_ = SDL_CreateTexture(
              SDL::getRenderer(video->monitor_), video->sdlFormat_,
...
std::string
GStreamerVideo::generateDotFileName(const std::string &prefix,
