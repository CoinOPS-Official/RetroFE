...
## Overview
The global `settings.conf` file, located in the root directory, controls the global behavior of the RetroFE instance. Below is a list of available configuration parameters you can define in this file. A default `settings.conf` can be generated by passing `--createconfig` as argument, see [CLI](CLI.md)
